...
static void NeoStripSetPixelColor(LEDStrip* strip, Length pixelN, Color* color) {
    RGB_t rgb = color->frameParameters.color;
    if (rgb.alpha) {
        strip->strip->setPixelColor(strip->position + pixelN, rgb.r, rgb.g, rgb.b);
    }
}
#ifdef ALLOW_GET_PIXEL
static void NeoStripGetPixelColor(LEDStrip* strip, Length pixelN, Color* color) {
    uint32_t c = strip->strip->getPixelColor(strip->position + pixelN);
    CONFIGURE_COLOR_RGB((*color), RGB((uint8_t)((c >> 16) & 0xFF), (uint8_t)((c >> 8) & 0xFF), (uint8_t)(c & 0xFF)));
}
#endif

void NeoStripInit(LEDStrip* strip, Adafruit_NeoPixel* baseStrip, const Length position, const Length length) {
  LEDStripInit(strip, length);
  strip->strip = baseStrip;
  strip->position = position;
  strip->setPixelColor = NeoStripSetPixelColor;
  #ifdef ALLOW_GET_PIXEL
  strip->getPixelColor = NeoStripGetPixelColor;
#endif
}
...
void flameFrame(LEDStrip* strip, FrameNumber* frame, FrameParameters* data) {
    if (frame->pi != frame->i) {
        ...
        Color color;
        byte d = data->parameter2 == 0 ? 1 : data->parameter2;
        for(Length i = 0; i < strip->length; i++) {
            uint8_t x = random(60) + 67;
            RGB_t rgb;
            for (byte c = 0; c < ARRAY_SIZE(data->color.grb); c++) {
              if (data->color.grb[c] == 255) {
                  rgb.grb[c] = min(x * ((i % k) == 0 ? 2 : 4), 255) / d;
...
void randomPixelFrame(LEDStrip* strip, FrameNumber* frame, FrameParameters* data) {
  ...
  CONFIGURE_COLOR_RGB(bg, background(data));
  LEDStripSetAllPixels(strip, &bg);
  LEDStripSetPixel(strip, random(strip->length), &color, data->inverted);
  nextFrame(frame, data->divider, 2);
}
